...
| tags | object | `{"autoscaler-anywhere":false,"autoscaler-openshift":false,"castai-cluster-agent":false,"full":false,"node-autoscaler":false,"readonly":false,"workload-autoscaler":false}` | Profile mode selection (mutually exclusive — pick one). Component overrides are stable across all mode upgrades with --reuse-values:   --set autoscaler.castai-kvisor.enabled=false |
