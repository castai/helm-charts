...
| castai | object | `{"apiKey":"","apiKeySecretRef":"","apiURL":"https://api.cast.ai","audit":{"createNamespace":true,"enabled":true,"namespace":"castai-audit"},"clm":{"enabled":false},"clusterIdConfigMapKeyRef":{"key":"CLUSTER_ID","name":""},"commitmentBudgets":{"enabled":true,"nodePoolFilterMode":"opt-out"},"continuousRebalancing":{"aggressiveModeConfig":{},"cycleIntervalSeconds":60,"deleteEmptyCycleIntervalSeconds":60,"drainOnlyCycleIntervalSeconds":60,"enabled":false,"evictionConfig":[],"fullModeCycleIntervalSeconds":1800,"minNodeAgeSeconds":300,"mode":"full","savingsThresholdPercent":15,"savingsThresholdPercentage":15},"grpcURL":"grpc.prod-master.cast.ai:443","organizationIdConfigMapKeyRef":{"key":"ORGANIZATION_ID","name":""},"otelMetrics":{"enabled":true},"rebalance":{"disruptGuard":true,"enabled":true,"namespaceLabeling":true,"preferenceWeightedPacking":true,"softTSCSpreading":false},"spotInterruptionPrediction":{"enabled":true}}` | CAST AI API configuration |
...
| castai.commitmentBudgets | object | `{"enabled":true,"nodePoolFilterMode":"opt-out"}` | CommitmentBudgets controller configuration. |
...
| castai.rebalance | object | `{"disruptGuard":true,"enabled":true,"namespaceLabeling":true,"preferenceWeightedPacking":true,"softTSCSpreading":false}` | Enable rebalance feature |
...
| castai.rebalance.softTSCSpreading | bool | `false` | Spread pods with soft (ScheduleAnyway) zone TopologySpreadConstraints across AZs during rebalancing/consolidation, instead of packing them wherever is cheapest. Hot-reloadable via kubectl-edit on the dynamic config ConfigMap. Changes consolidation aggressiveness for existing customers, so it needs explicit opt-in. Default: false. |
