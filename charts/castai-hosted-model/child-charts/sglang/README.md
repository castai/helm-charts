...
| router.strategy | object | `{"rollingUpdate":{"maxSurge":1,"maxUnavailable":0},"type":"RollingUpdate"}` | Deployment rollout strategy for the router, applied at ANY replica count.   type: RollingUpdate | Recreate   rollingUpdate.maxSurge / maxUnavailable: only used when type=RollingUpdate. The default (RollingUpdate, maxSurge:1, maxUnavailable:0) gives a zero-downtime replace even at replicaCount=1: the new router becomes Ready before the old one is torn down. (This briefly runs two routers during a rollout; they don't share the cache-aware radix tree, so cache-hit dips for the few seconds of overlap. Set type=Recreate for a clean single-router replace instead, at the cost of a brief serving gap on deploy.) |
