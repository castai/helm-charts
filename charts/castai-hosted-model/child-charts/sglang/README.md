...
| router.podAntiAffinity | object | `{"enabled":true,"topologyKey":"kubernetes.io/hostname","type":"required"}` | Spread router replicas across nodes. Only takes effect when replicaCount > 1 (a single router has nothing to separate from). Keeps the replicas off the same node so one node |
...
| router.strategy | object | `{"rollingUpdate":{"maxSurge":1,"maxUnavailable":0},"type":"RollingUpdate"}` | Deployment strategy. Use RollingUpdate when replicaCount > 1; when replicaCount == 1 the chart picks the single-replica strategy based on zeroDowntimeSingleReplica below. |
