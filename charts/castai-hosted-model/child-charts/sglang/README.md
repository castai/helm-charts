...
| router.strategy | object | `{"rollingUpdate":{"maxSurge":1,"maxUnavailable":0},"type":"RollingUpdate"}` | Deployment strategy. Use RollingUpdate when replicaCount > 1; the chart forces Recreate when replicaCount == 1 (single in-memory router, avoid two competing instances). |
...
| tolerations | list | `[]` |  |
| tracing | object | `{"enabled":false,"level":"","otlpEndpoint":"observability-otel-gateway-opentelemetry-collector.tools.svc.cluster.local:4318","protocol":"http/protobuf","serviceName":""}` | End-to-end inference tracing. When enabled, both the worker (sglang.launch_server) and, if deployed, the router (sglang_router.launch_router) are launched with --enable-trace and export OTLP spans to otlpEndpoint. NOTE: SGLang does not continue an upstream W3C traceparent — it starts its own root trace per request. Spans correlate to the caller by request id, not parent span. |
| tracing.level | string | `""` | Optional trace verbosity (0-3). Empty leaves the SGLang default. |
| tracing.otlpEndpoint | string | `"observability-otel-gateway-opentelemetry-collector.tools.svc.cluster.local:4318"` | OTLP collector endpoint (host:port). Defaults to the shared observability gateway (same collector the LLM proxy uses). For OTLP/HTTP use the :4318 port and protocol http/protobuf; for OTLP/gRPC use :4317 and protocol grpc. |
| tracing.protocol | string | `"http/protobuf"` | Exporter protocol: "http/protobuf" (pairs with :4318) or "grpc" (pairs with :4317). SGLang's --otlp-traces-endpoint defaults to gRPC, so this is set explicitly via OTEL_EXPORTER_OTLP_TRACES_PROTOCOL to match the endpoint above. |
| tracing.serviceName | string | `""` | Optional service name reported on spans. Defaults to the served model name when empty. |
